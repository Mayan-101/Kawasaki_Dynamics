...
static double measure_M_cpu(const int *grid)
{
    long long sum = 0;
...
void run_hysteresis(double T, double h_max, int h_steps, int equil_sweeps, int meas_sweeps, IsingGpuPlugin *gpu, IsingGpuState *gpu_state, int *host_grid)
{
    FILE *fp = fopen("ising_gpu_hysteresis.csv", "w");
    if (!fp) { fprintf(stderr, "Cannot open ising_gpu_hysteresis.csv\n"); return; }

    fprintf(fp, "h,M,branch\n");

    printf("Ising Hysteresis sweep  T=%.4f  h_max=%.4f  h_steps=%d"
           "  equil=%d  meas=%d\n", T, h_max, h_steps, equil_sweeps, meas_sweeps);
    printf("Output -> ising_gpu_hysteresis.csv\n");
    ...
    fclose(fp);
    printf("Saved ising_gpu_hysteresis.csv\n");
}
...
#pragma omp single
    ...
    IsingGpuPlugin gpu = {0};
    bool use_gpu = ising_gpu_plugin_load(&gpu, "ising.dll");

    int *host_grid = (int *)malloc(HEIGHT * WIDTH * sizeof(int));
    if (!host_grid)
    {
